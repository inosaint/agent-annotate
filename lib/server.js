...
function createServer(opts = {}) {
  ...
  const store = path.resolve(opts.store || path.join(root, 'annotations.json'));
  ...
  const shotDir = path.resolve(opts.shotDir || store.replace(/\.json$/, '-shots'));
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (u.pathname === '/__annotate/client.js') {
      ...
      const cfg = JSON.stringify({ ignoreClasses: opts.ignoreClasses || [] });
    ...
    if (u.pathname === '/__annotations/handoff') {
      if (req.method === 'GET') {
        return json(res, 200, {
          configured: !!trigger, command: trigger, agent,
      ...
      try {
        ...
        if (sent.length && !trigger) log('    (nothing configured to run — pass --agent or --on-handoff)');
        if (sent.length && trigger) runHandoff(sent.map(a => a.id));
        return json(res, 200, {
          ok: true, sent: sent.length, configured: !!trigger, agent,
  ...
  return Object.assign(server, { root, store, resolved, index, onHandoff, agent, agentLog, shotDir });
}
